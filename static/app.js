...
function cambiarVistaContactos(vista) {
  ...
  document.getElementById("btnContVista1").style.backgroundColor =
    vista === 1 ? "#333" : "#777";
  document.getElementById("btnContVista2").style.backgroundColor =
    vista === 2 ? "#333" : "#777";
  cargarDatos("contactos");
}
...
function renderizarProductos(result) {
  ...
  document.getElementById(
    "infoPaginacion"
  ).innerText = `Página ${paginaActualProducto + 1} de ${totalPages}`;
...
function renderizarContactos(result) {
  ...
  const totalPages = Math.ceil(count / pageSize) || 1;
  document.getElementById(
    "infoPaginacionContacto"
  ).innerText = `Página ${paginaActualContacto + 1} de ${totalPages}`;
...
function irAPaginaContacto() {
  const pag = parseInt(
    document.getElementById("inputPaginaContacto").value,
    10
  );
  if (pag > 0) {
